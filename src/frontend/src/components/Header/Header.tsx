...
function HeaderUserSection() {
	if (loggedInUser$.value === notLoggedIn || loggedInUser$.value.state === 'has error') {
		return <LoginButton />;
	}

	if (loggedInUser$.value.state === 'loading') {
		return (
			<Avatar
				label='Loading user...'
				class='he_avatar'
	...
	const menuHandler = (event: SlSelectEvent) => {
	...
	const user = loggedInUser$.value.data;

	return (
		<Dropdown>
			<button slot='trigger' class='he_avatar-button'>
				<Avatar
					class='he_avatar'
					title={user.name}
...
export function Header({ class: _class }: HeaderProps) {
	return (
		<div class={cc(_class, 'he')}>
			...
			<SearchBar class='he_searchbar' />
			<div class='he_spacer' aria-hidden />
			<div class='he_user-wrapper'>
				<HeaderUserSection />
			</div>
		</div>
	);
