...
from orchard.libs.hash import sha256, sha1
...
class BunnyStorage:
    ...
    def __init__(self, api_key: str, base_endpoint: str, storage_zone_name: str, public_cdn_base: str):
        """
        Initialize the BunnyStorage object. This should always be used as part of an async
        context manager, e.g.:

        ```
        async with BunnyStorage(...) as bun:
            pass
        ```
        """
        ...
        self.client = AsyncClient(timeout=7.0) # it's the number!
    ...
    def _get_hash_path_and_file_name(self, file: BinaryIO, path: str, file_extension: str):
    ...
    def get_url_by_hash(self, file: BinaryIO, path: str, file_extension: str):
        path, file_name = self._get_hash_path_and_file_name(file, path, file_extension)
    ...
    async def upload_file_by_hash(self, file: BinaryIO, path: str, file_extension: str):
        "two directories for the first four chars of the hash. which corresponds to 65536 total."
        file_hash = sha1(file)
        dir1, dir2, rest = file_hash[0:2], file_hash[2:4], file_hash[4:]
