...
from orchard.libs.vitals.color_tagged_string import ColorToken, parse_color_tagged_string
...
def test_parse_color_tagged_string_returns_string_without_tags(color_tagged1):
    ...
    assert t1 == [
        ColorToken(len=6, color="default"),
        ColorToken(len=5, color="blue")
    ]
...
def test_parse_color_tagged_string_works_with_trailing_untagged_text():
    ...
    assert t1 == [
        ColorToken(len=11, color="default"),
        ColorToken(len=4, color="blue"),
        ColorToken(len=24, color="default")
    ]
...
def test_parse_color_tagged_string_works_with_unclosed_color_tag():
    ...
    assert t1 == [
        ColorToken(len=len(s1), color="#ababab")
    ]
...
def test_parse_nested_strings():
    ...
    assert t1 == [
        ColorToken(len=6, color="default"),
        ColorToken(len=16, color="red"),
        ColorToken(len=14, color="blue"),
        ColorToken(len=11, color="red"),
        ColorToken(len=27, color="default")
    ]
