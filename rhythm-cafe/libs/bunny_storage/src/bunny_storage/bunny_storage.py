...
class BunnyStorage:
    ...
    def upload_file(self, file: BinaryIO, path: str, file_name: str, skip_if_already_exists: bool):
        if skip_if_already_exists:
            already_exists = self.file_exists(path, file_name)
            if already_exists:
                logger.info(f"Skipping upload of {path}/{file_name} as it already exists.")
                return
        url = self._build_path(path, file_name)
        file_hash = sha256(file).upper()
        self.client.put(url, headers={
            "AccessKey": self.api_key,
            "Checksum": file_hash
