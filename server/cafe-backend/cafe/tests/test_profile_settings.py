...
@pytest.mark.django_db
def test_profile_settings_get_method_only_returns_data(bridge_client: Client, user_with_no_clubs):
    """Test that GET requests don't modify data and return current state"""
    bridge_client.force_login(user_with_no_clubs)
    ...
    # Multiple GET requests should return the same data without changes
    for _ in range(3):
        response = bridge_client.get('/accounts/profile/settings/')
        assert response.status_code == 200
        assert response.json()['context']['user']['theme_preference'] == 'dark'
...
@pytest.mark.django_db
def test_profile_settings_invalid_http_methods(bridge_client: Client, user_with_no_clubs):
    """Test that unsupported HTTP methods still return a response"""
    bridge_client.force_login(user_with_no_clubs)
    
    # PUT, PATCH, DELETE should all return the normal response (not modify data)
    for method in ['put', 'patch', 'delete']:
        response = getattr(bridge_client, method)('/accounts/profile/settings/', {
            'theme_preference': 'dark',
        ...
        get_response = bridge_client.get('/accounts/profile/settings/')
        assert get_response.json()['context']['user']['theme_preference'] == 'light'  # default
        assert get_response.json()['context']['user']['displayName'] == ''  # default


@pytest.mark.django_db
def test_profile_settings_missing_csrf_token_handling(bridge_client: Client, user_with_no_clubs):
    """Test that the view handles requests without CSRF token appropriately"""
    # Note: bridge_client likely handles CSRF automatically, but this tests the view's robustness
    bridge_client.force_login(user_with_no_clubs)
    ...
    # Should still work (bridge_client handles CSRF)
    assert response.status_code == 200