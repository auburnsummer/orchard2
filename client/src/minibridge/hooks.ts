...
type ExtractSetAtom<A> =
  A extends SetAtom<infer Args, infer Result>
    ? (...args: Args) => Result
    : never;
...
export const useSearchParams = () => {
  ...
  const navigateViaSearchParams = useCallback(
    (func: (params: URLSearchParams) => void | URLSearchParams) => {
      ...
      navigate(
        makeCanonicalURL(
          `${location.pathname}?${searchParamsToUse.toString()}`,
        ),
      );
    },
    [searchParams],
