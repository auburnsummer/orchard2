...
export function Toggle({
  ...
  return (
    <div className={cc("flex items-center justify-between gap-3", className)}>
      <div className="group relative inline-flex w-11 shrink-0 rounded-full bg-slate-200 p-0.5 inset-ring inset-ring-slate-900/5 outline-offset-2 outline-violet-600 transition-colors duration-200 ease-in-out has-checked:bg-violet-600 has-focus-visible:outline-2 dark:bg-white/5 dark:inset-ring-white/10 dark:outline-violet-500 dark:has-checked:bg-violet-500">
      ...
      {(label || description) && (
        <div className="text-sm">
          {label && (
            <label id={labelId} htmlFor={id} className={cc("font-medium text-slate-900 dark:text-white", labelClassName)}>
              {label}
            </label>
          )}
          {label && description && ' '}
