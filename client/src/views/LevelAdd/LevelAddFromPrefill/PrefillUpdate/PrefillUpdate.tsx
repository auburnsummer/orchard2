            ...
            <Box p="md">
                <Alert icon={<FontAwesomeIcon icon={faInfoCircle} />}>
                    ...
                    <List>
                        ...
                        <ListItem><Text>We have also guessed potential levels for your rdzip in the "Potential Matches" section below. If your level is there, you can click it, then click Submit.</Text></ListItem>
                    </List>
                ...
                <Form method="POST">
                    { csrfInput }
                    <Group align="end">
                        <TextInput label="Level ID" placeholder="Enter level ID" value={levelId} onChange={(e) => setLevelId(e.currentTarget.value)} name="prefill" />
                        <Button disabled={!levelId} type="submit">Submit</Button>
                    </Group>
                </Form>
                ...
                {potentialMatches.length > 0 && (
                    <List>
                        {potentialMatches.map((match) => (
                            <LevelCard key={match.id} level={match} showId={true} onClick={() => setLevelId(match.id)} />
                        ))}
                    </List>
                )}
            </Box>
